...
const BlowMoulding = () => {
  const {plant} = useRouter().query

  useEffect(() => {
    setTimeout(() => {
      toast('Under Development. Only the design built at the moment.')
    }, 3000);
  }, [])
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: 1 }}>
      ...
      <div className="relative w-full">
        ...
        <div className="absolute w-full top-0 left-0 bottom-0 flex text-white">
          <div className="flex flex-col ml-36 my-auto">
            <h1 className={classNames(PlayfairDisplayBold.className, 'text-7xl')}><span className='capitalize'>{plant} Plant</span></h1>
      ...
      <div className={classNames('px-14 py-4 bg-[#fff4dc] text-xl', InterBold.className)}>
        HOME <span className='text-blue-400'>{">"}</span> CAREERS <span className='text-blue-400'>{">"}</span> <span className='uppercase'>{plant} PLANT</span>
      </div>
